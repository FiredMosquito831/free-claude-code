...
async def test_stop_is_idempotent_without_a_live_process() -> None:
    ...
    with (
        ...
        patch("my_claude_code.cli.managed.session.unregister_pid") as unregister,
    ):
        assert await session.stop() is True
...
async def test_stopped_session_reference_cannot_launch_a_new_process() -> None:
    ...
    with (
        patch(
            "my_claude_code.cli.managed.session.asyncio.create_subprocess_exec",
            new=AsyncMock(return_value=_completed_process(100)),
        ) as create_process,
        patch("my_claude_code.cli.managed.session.trace_event") as trace,
...
async def test_launch_publication_wins_before_concurrent_stop() -> None:
    ...
    with (
        ...
        patch("my_claude_code.cli.managed.session.kill_pid_tree_best_effort"),
        patch("my_claude_code.cli.managed.session.register_pid"),
        patch("my_claude_code.cli.managed.session.unregister_pid"),
    ):
...
async def test_task_failure_keeps_unconfirmed_process_pid_registered() -> None:
    ...
    with (
        ...
        patch("my_claude_code.cli.managed.session.register_pid") as register,
        patch("my_claude_code.cli.managed.session.unregister_pid") as unregister,
