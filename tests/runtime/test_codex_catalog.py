...
from my_claude_code.application.ports import (
    RequestRuntimeLease,
    RequestRuntimePort,
)
from my_claude_code.config.settings import Settings
from my_claude_code.runtime.codex_catalog import CodexModelCatalogPublisher
...
def test_empty_projection_preserves_existing_catalog(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "my_claude_code.runtime.codex_catalog.build_codex_model_catalog",
            return_value={"models": []},
        ),
...
def test_resolved_catalog_path_defaults_to_fcc_config(tmp_path: Path) -> None:
    publisher = CodexModelCatalogPublisher()

    with patch(
        "my_claude_code.runtime.codex_catalog.codex_model_catalog_path",
        return_value=tmp_path / "defaulted.json",
    ) as defaulted:
        resolved = publisher._resolved_catalog_path()

    defaulted.assert_called_once_with()
    assert resolved == tmp_path / "defaulted.json"
